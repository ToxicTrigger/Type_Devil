...
public abstract class NeuronDocker {
	...
	if (!sendDataToOtherNeuronsTmpInData(out, in))
	    return false;
	...
	if (!(out.hasDocked() & in.hasDocked()))
	    return false;
	if (!out.getData().equals(in.getData()))
	    return false;
	return true;
    }
	...
	if (in == null)
	    return false;
	if (in.getState() != NeuronState.Idle)
	    return false;
	in.setState(NeuronState.Finding);
	if (out == null)
	    return false;
	if (out.getState() != NeuronState.Idle)
	    return false;
	out.setState(NeuronState.Finding);

	if (in.getState() == NeuronState.Finding & out.getState() == NeuronState.Finding)
	    return true;
	return false;
    }

    public static boolean sendDataToOtherNeuronsTmpInData(Neuron out, Neuron in) {
	if (in.hasDocked() == false || out.hasDocked() == false)
	    return false;
	if (!(out.sendDataToTmpData() & in.getDataFromOutsTmpData(out)))
	    return false;
	return true;
    }
