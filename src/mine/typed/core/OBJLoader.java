...
/**
 * .obj 파일을 로드 하기 위한 로더 입니다. 3D 모델을 불러 옵니다.
 * 
 * @author mrminer
 *
 */
public class OBJLoader {
	...
	try {
		...
		if (line.startsWith("f ")) {
		    ...
		    facesVerts[faceIndex] = OBJLoader
			    .getIndex(parts[0], numVertices);
		    if (parts.length > 2) {
			facesNormals[faceIndex] = OBJLoader
				.getIndex(parts[2], numNormals);
		    }
		    if (parts.length > 1) {
			facesUV[faceIndex] = OBJLoader
				.getIndex(parts[1], numUV);
		    }
		    faceIndex++;
	    ...
	    final float[] verts = new float[numFaces * 3 * (3 + (numNormals > 0 ? 3 : 0) + (numUV > 0 ? 2 : 0))];
	    ...
	    final Vertices3 model = new Vertices3(game.getGLGraphics(), numFaces * 3, 0, false, numUV > 0, numNormals > 0);
