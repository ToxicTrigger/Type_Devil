...
public abstract class Button extends UI {
	...
	if (OverlapTester.pointInRectangle(this.hitbox, touchPos)) {
	    if (touchType == TouchEvent.TOUCH_DOWN | touchType == TouchEvent.TOUCH_DRAGGED)
		this.buttonState = Button.PUSHING;
	...
	if (!OverlapTester.pointInRectangle(this.hitbox, touchPos) & (touchType == TouchEvent.TOUCH_UP | touchType == TouchEvent.TOUCH_DOWN | touchType == TouchEvent.TOUCH_DRAGGED)) {
	    this.buttonState = Button.DISABLED;
	}
	...
	if (this.hitbox.getState() == HitBox.STATE_CHECK) {
	    ...
	    runEvent();
	    if (this.buttonState == Button.ACTIVATION | (!OverlapTester
		    .pointInRectangle(this.hitbox, touchPos) & eventType == TouchEvent.TOUCH_UP))
		this.buttonState = Button.DISABLED;
	}
    }
