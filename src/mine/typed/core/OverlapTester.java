...
public class OverlapTester {
	...
	return (h1.lowerLeft.x < (h2.lowerLeft.x + h2.width)) && ((h1.lowerLeft.x + h1.width) > h2.lowerLeft.x) && (h1.lowerLeft.y < (h2.lowerLeft.y + h2.height)) && ((h1.lowerLeft.y + h1.height) > h2.lowerLeft.y);

    }

    public static boolean overlapHitBoxAndRectangle(final HitBox h,
	    final Rectangle r) {
	if (h.getState() == HitBox.STATE_UNCHECK)
	    return false;
	...
	return (h.lowerLeft.x < (h.lowerLeft.x + h.width)) && ((h.lowerLeft.x + h.width) > h.lowerLeft.x) && (h.lowerLeft.y < (h.lowerLeft.y + h.height)) && ((h.lowerLeft.y + h.height) > h.lowerLeft.y);
    }
     ...
     * @return 좌표에 구가 포함된다면 true , 아니라면 false
     */
    public static boolean pointInSphere(final Sphere c, final float x,
	...
	return (h.lowerLeft.x <= p.x) && ((h.lowerLeft.x + h.width) >= p.x) && (h.lowerLeft.y <= p.y) && ((h.lowerLeft.y + h.height) >= p.y);
    }
    ...
    public static boolean pointInHitBox(final HitBox h, final float x,
	    final float y) {
	...
	return (h.lowerLeft.x <= x) && ((h.lowerLeft.x + h.width) >= x) && (h.lowerLeft.y <= y) && ((h.lowerLeft.y + h.height) >= y);
    }
	...
	if (point.x > box.LeftEdgeUp.x)
	    return false;
	if (point.x < box.ReightEdgeDown.x)
	    return false;
	if (point.y > box.LeftEdgeUp.y)
	    return false;
	if (point.y > box.ReightEdgeDown.y)
	    return false;
	if (point.z > box.LeftEdgeUp.z)
	    return false;
	if (point.z > box.ReightEdgeDown.z)
	    return false;
	return true;
    }
	...
	if ((_t == 0) && (_s == 0))
	    return tmp;
	return tmp
		.set((float) (line1.start.x + (t * (line1.end.x - line1.start.x))), (float) (line1.start.y + (t * (line1.end.y - line1.start.y))));
    }
