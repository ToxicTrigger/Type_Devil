...
/**
 * 해당 클래스는 화면을 여러개의 셀로 나누고 그 셀에 포함된 것들만 계산하도록 해주는 클래스 입니다.
 * 충돌처리등 비교적 연산비용이 큰 것들을 보다 저렴하고 효과적으로 관리할수 있습니다.
 * @author mrminer
 *
 */
...
public class SpatialHashGrid {
	...
	public int[ ] getCellIds(final GameObject obj) {
		...
		final int x1 = (int) FloatMath
				.floor( obj.bounds.lowerLeft.x / this.cellSize );
		final int y1 = (int) FloatMath
				.floor( obj.bounds.lowerLeft.y / this.cellSize );
		final int x2 = (int) FloatMath
				.floor( (obj.bounds.lowerLeft.x + obj.bounds.width) / this.cellSize );
		final int y2 = (int) FloatMath
				.floor( (obj.bounds.lowerLeft.y + obj.bounds.height) / this.cellSize );

		if ( (x1 == x2) && (y1 == y2) ) {
			if ( (x1 >= 0) && (x1 < this.cellsPerRow) && (y1 >= 0)
					&& (y1 < this.cellsPerCol) ) {
			...
			if ( (x2 >= 0) && (x2 < this.cellsPerRow) && (y1 >= 0)
					&& (y1 < this.cellsPerCol) ) {
				this.cellIds[ i++ ] = x2 + y1CellsPerRow;
			}
			if ( (x2 >= 0) && (x2 < this.cellsPerRow) && (y2 >= 0)
					&& (y2 < this.cellsPerCol) ) {
				this.cellIds[ i++ ] = x2 + y2CellsPerRow;
			}
			if ( (x1 >= 0) && (x1 < this.cellsPerRow) && (y2 >= 0)
					&& (y2 < this.cellsPerCol) ) {
