...
/**
 * 초기 설정값을 정할수 있습니다. FBO.W , FBO.H 를 수정하세요. 기초값은 W = 800 , H = 480 입니다.
 * 
 * @author mrminer
 *
 */
public abstract class FBO {
    ...
    static public boolean checkIfContextSupportsExtension(GL10 gl,
	    String extension) {
    ...
    public static void onSurfaceCreated(GLGame glga, GL10 gl,
	    javax.microedition.khronos.egl.EGLConfig config) {
	FBO.mContextSupportsFrameBufferObject = FBO
		.checkIfContextSupportsFrameBufferObject(gl);

	if (FBO.mContextSupportsFrameBufferObject) {
	    FBO.mTargetTexture = FBO
		    .createTargetTexture(gl, FBO.mWidth, FBO.mHeight);
	    FBO.mFramebuffer = FBO
		    .createFrameBuffer(gl, FBO.mWidth, FBO.mHeight, FBO.mTargetTexture);
