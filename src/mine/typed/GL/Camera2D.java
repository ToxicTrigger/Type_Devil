...
public class Camera2D {
	 ...
	 * @param frustumHeight
	 */
	public Camera2D( final GLGraphics glG, final float frustumWidth, final float frustumHeight ) {
	...
	public void setViewportAndMatrices( ) {
		...
		gl.glLoadIdentity( );
		gl.glOrthof( this.position.x - ((this.frustumWidth * this.zoom) / 2) , this.position.x
				+ ((this.frustumWidth * this.zoom) / 2) , this.position.y
				- ((this.frustumHeight * this.zoom) / 2) , this.position.y
				+ ((this.frustumHeight * this.zoom) / 2) , 1 , -1 );
	...
	public void touchToWorld(final V2 touch) {
		...
		touch.add( this.position ).sub( (this.frustumWidth * this.zoom) / 2 ,
				(this.frustumHeight * this.zoom) / 2 );
	}
}
