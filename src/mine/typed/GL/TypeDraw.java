...
/**
 * 이 클래스는 여러가지 드로우 기능을 제공한다. 사각형 그리기나 선 , 그리고 이미지에 색을 입히는 작업이나 투명도를 제어하기 쉽게
 * 만들어줄 목적으로 만들어졌다.
 */
public class TypeDraw {
	...
	public void makeRect( final Rectangle rect) {
		...
		c.drawRect( rect.lowerLeft.x , rect.lowerLeft.y , rect.width ,
	 ...
	 * @param gl
	 */
	public void drawTexture( final SpriteBatcher sb,  final float x,  final float y,  final float w,
			final float h,  final float angle,  final float A,  final float R,  final float G,  final float B,  final GL10 gl) {
	...
	public void drawTextureRegion( final Texture T,  final float x,  final float y,  final float w,
			final float h,  final float a,  final float r,  final float g,  final float b,  final float angle,  final GL10 gl,
			final TextureRegion Tr ,  final boolean draw) {
		if(draw){
			gl.glColor4f( r , g , b , a );
